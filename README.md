...
日付は必ずJSTで記録します。GitHub Actions のコンテナはUTCのため
`date.today()` を使うと記事の表示日付(JST)とズレるうえ、スケジュール実行が
遅延してUTCの日付をまたぐと履歴に穴が空きます
（実際に `08-25T21:30Z` と `08-27T00:32Z` の実行で `08-26` が欠落しました）。
